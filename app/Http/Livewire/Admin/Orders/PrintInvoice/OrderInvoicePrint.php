...
{
    public $order,$orderdetails,$orderaddons,$balance,$total,$customer,$payments,$sitename,$address,$phone,$paid_amount,$payment_type,$zipcode,
    $tax_number,$store_email,$from_date,$to_date,$quantity,$is_invoice_paid,$total_paid_amount,$remain_amount;
    /* render the page */
    ...
    public function mount($id)
    {
        ...
        {
            $this->quantity += $item->service_quantity;
        }
     

        $this->payments = Payment::where('order_id',$this->order->id)->get();
        $this->total_paid_amount = DB::table('payments')->where('order_id','=',$this->order->id)->sum('received_amount');
        $this->is_invoice_paid = $this->total_paid_amount == $this->order->total ? 'Paid' : 'Not Paid';
        $this->remain_amount = $this->order->total > $this->total_paid_amount ? $this->order->total - $this->total_paid_amount : 0;
